...
public class ActivityRepository : IActivityRepository
{
    ...
    {
        return await _dbContext.Activities.Include(_ => _.Expenses).OrderBy(_ => _.Id).ToListAsync();
    }

    public async Task<Activity> GetAsync(long id, CancellationToken cancellationToken = default)
    {
        var activity = await _dbContext.Activities.Include(_ => _.Expenses).FirstOrDefaultAsync(e => e.Id == id, cancellationToken);
        if (activity == null)
